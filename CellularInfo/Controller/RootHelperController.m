#import "RootHelperController.h"
#include <spawn.h>
#import <sys/sysctl.h>
#import <Foundation/Foundation.h>

@implementation RootHelperController
...
int spawnRoot(NSString* path, NSArray* args, NSString** stdOut, NSString** stdErr)
{
    ...
    pid_t task_pid;
    int status = -200;
    int spawnError = posix_spawn(&task_pid, [path UTF8String], &action, attrPtr, (char* const*)argsC, NULL);
...
int killallWithResult(NSString* processName, BOOL softly)
{
    ...
    enumerateProcessesUsingBlock(^(pid_t pid, NSString* executablePath, BOOL* stop)
    {
        if([executablePath.lastPathComponent isEqualToString:processName])
        {
            int signal = softly ? SIGTERM : SIGKILL;
